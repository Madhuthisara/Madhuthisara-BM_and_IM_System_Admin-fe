...
export const materialService = {
    getAllMaterials: async (businessId: string, config?: AxiosRequestConfig): Promise<MaterialsListResponse> => {
        const response = await axiosInstance.get(`${API_ENDPOINTS.MATERIALS.ALL}?business_id=${businessId}`, config);
