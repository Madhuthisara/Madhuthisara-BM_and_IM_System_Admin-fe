...
export const orderService = {
    getAllOrders: async (businessId: string, config?: AxiosRequestConfig): Promise<OrdersListResponse> => {
        const response = await axiosInstance.get(`${API_ENDPOINTS.ORDERS.ALL}?business_id=${businessId}`, config);
