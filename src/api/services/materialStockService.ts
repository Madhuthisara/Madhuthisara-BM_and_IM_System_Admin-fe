...
export const materialStockService = {
    getAllMaterialStocks: async (businessId: string, config?: AxiosRequestConfig): Promise<MaterialStocksListResponse> => {
        const response = await axiosInstance.get(`${API_ENDPOINTS.MATERIAL_STOCKS.ALL}?business_id=${businessId}`, config);
