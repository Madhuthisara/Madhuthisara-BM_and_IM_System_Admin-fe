...
export const attributeService = {
    getAllAttributes: async (businessId: string, config?: AxiosRequestConfig): Promise<AttributesListResponse> => {
        const response = await axiosInstance.get(`${API_ENDPOINTS.ATTRIBUTES.ALL}?business_id=${businessId}`, config);
    ...
    getAllOptions: async (attributeId: string, config?: AxiosRequestConfig): Promise<AttributeOptionsListResponse> => {
        const response = await axiosInstance.get(`${API_ENDPOINTS.ATTRIBUTES.OPTIONS.ALL}?attribute_id=${attributeId}`, config);
        return response.data;
    },
