...
const Products: React.FC = () => {
    ...
    const handleEdit = (record: ProductData) => {
        ...
        form.setFieldsValue({ name: record.name, sku: record.sku });
        setIsModalOpen(true);
    };
        ...
        form.validateFields().then((values) => {
            if (editingKey) {
                const newData = data.map((item) =>
                    item.key === editingKey ? { ...item, name: values.name, sku: values.sku } : item
                );
                setData(newData);
            } else {
                const newData: ProductData = { key: Date.now().toString(), name: values.name, sku: values.sku };
    ...
    return (
        <MasterDataSubPageLayout title="Products" onAdd={handleAdd} addButtonText="Add Product">
            <div style={{ marginBottom: 24 }}><Text type="secondary">Manage final products for sale.</Text></div>
            <Card bordered={false} style={{ borderRadius: 8 }}>
                <div style={{ marginBottom: 16 }}><Title level={4}>Products</Title><Text type="secondary">A list of all products.</Text></div>
