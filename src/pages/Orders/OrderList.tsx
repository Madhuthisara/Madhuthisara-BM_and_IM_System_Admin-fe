...
const OrderList: React.FC = () => {
    ...
    const { data: ordersData, isLoading } = useQuery({
        ...
        queryFn: () => orderService.getAllOrders(businessId),
        enabled: !!businessId,
    });
    ...
    const customStatuses = customStatusesData?.output || [];
    ...
    const filteredOrders = useMemo(() => {
        if (!ordersData?.output) return [];

        return ordersData.output.filter(order => {
            const matchesStatus =
                statusFilter === 'all' ||
